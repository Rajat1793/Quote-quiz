import requests
from bs4 import BeautifulSoup
from csv import writer
from time import sleep 
from random import choice
from csv import DictReader



BASE_URL = "http://quotes.toscrape.com"

def read_quotes(filename):
    with open(filename, "r") as file:
        csv_reader =DictReader(file)
        return list(csv_reader)


def start_game(quotes):
    quote = choice(quotes)
    guess_remaining = 4
    print("Quote:",quote["text"])
    print("Author:",quote["author"])
    guess = ''
    while guess.lower() != quote["author"].lower() and guess_remaining >0:
        guess = input(f"who said this quote? Guess remaining {guess_remaining}\n")
        if guess.lower() == quote["author"].lower():
            print("You got it right!!!")
            break
        guess_remaining -= 1
        if guess_remaining == 3:
            res = requests.get(f"{BASE_URL}{quote['bio-link']}")
            soup = BeautifulSoup(res.text, "html.parser")
            birth_date = soup.find(class_="author-born-date").get_text()
            birth_place = soup.find(class_="author-born-location").get_text()
            print(f"Here's a hint: The author was born on {birth_date} {birth_place} ")
        elif guess_remaining == 2:
            print(f"Here's a hint: The author first name starts with: {quote['author'][0]} ")
        elif guess_remaining == 1:
            last_initial = quote["author"].split(" ")[1][0]
            print(f"Here's a hint: The author last name starts with: {last_initial} ")
        else:
            print(f"You ran out of the gusses. The answer was {quote['author']} ")

    again = ' '
    while again not in ('y', "Yes", 'n', 'no'):
        again = input("Would you like to play again (y/n)")
    if again.lower() in ("yes", 'y'):
        return start_game(quotes)
    else:
        print("Ok, BYE!")    

quotes = read_quotes("quotes.csv")
start_game(quotes)